...
import { OpenClawGatewayCliClient } from './gateway-client.js';
function splitIntoChunks(content, size = 180) {
    if (!content) {
        return [];
    }
    const chunks = [];
...
function shouldTreatAsAbort(errorMessage) {
...
export class WebChannelServer {
    ...
    setupRoutes() {
        ...
        this.app.get('/sessions/:sessionId/history', async (req, res) => {
            ...
            try {
                ...
                res.status(500).json(createErrorPayload('HIST_002', 'Failed to load history', {
                    sessionId,
                    recoverable: true,
                    action: 'retry'
                }));
        ...
        this.app.post('/auth', (req, res) => {
            const { apiKey, accountId = 'default', userId = 'web-user' } = req.body;
            ...
            const token = generateToken({
                ...
                session: randomUUID(),
                accountId
            }, this.config.auth.secret, this.config.auth.expiration);
            res.json({
                token,
                type: 'Bearer',
    ...
    async replayCachedResponse(client, messageId, cached) {
        ...
        client.ws.send(JSON.stringify(createErrorPayload(error.code, error.message, {
            messageId,
    ...
    async handleSocketMessage(client, raw) {
        let message;
        try {
    ...
    async handleChatMessage(client, message) {
        ...
        try {
            ...
            if (aborted) {
                ...
                const recoverable = !lowerError.includes('invalid token') &&
                ...
                client.ws.send(JSON.stringify(createErrorPayload('MSG_004', 'Failed to process message', {
                    messageId,
                    reason: errorMessage,
                    recoverable,
                    action
