...
function readAccounts(config: unknown): Record<string, Record<string, unknown>> {
  const channels =
    config && typeof config === 'object' && 'channels' in config
      ? (config as { channels?: Record<string, { accounts?: Record<string, Record<string, unknown>> }> }).channels
      : undefined;

  return channels?.['web-channel']?.accounts ?? {};
}

export function createWebChannel(config: PluginConfig, logger: Logger): ChannelPlugin {
  return {
    id: 'web-channel',
    meta: {
      id: 'web-channel',
    ...
    config: {
      ...
      resolveAccount(cfg, accountId): Record<string, unknown> {
